...
THNTensor *nn_Reshape_updateOutput(struct module *module, THNTensor *input)
{
	...
	if(module->Reshape.batchMode == 0 ||
		...
		THNTensor_resize(module->output, module->Reshape.size, module->Reshape.nsize);
	else {
		module->Reshape.batchsize[0] = input->size[0];
		THNTensor_resize(module->output, module->Reshape.batchsize, module->Reshape.nbatchsize);
	}
